...
function renderDashboard() {
    ...
    state.dashboardData.forEach(group => {
        // 👉 TỰ ĐỘNG SẮP XẾP LẠI THEO THỜI GIAN THỰC ĐƯỢC TÍCH HỢP Ở ĐÂY
        ...
        // Chặn render dữ liệu nhạy cảm nếu nhóm đang bị khóa (Bảo mật nâng cao)
        if (isLockedCheck) {
...
function moveItem(type, sourceGroupId, index, targetGroupId) {
    ...
    if (movedItem) {
        ...
        if (type === 'schedule') {
            sortSchedulesSmart(targetGroup.schedules);
        }
        saveData();
    }
}
...
function updateScheduleUI() {
    ...
    rows.forEach(row => {
        ...
        // 👉 SỬA LỖI MOBILE: Thay thế 'T' bằng khoảng trắng và thay '-' bằng '/' 
        // giúp tất cả trình duyệt Mobile (Safari/Android) hiểu đúng Múi giờ địa phương thay vì UTC
        ...
        // Kiểm tra nếu ngày tháng bị lỗi cấu trúc (NaN)
...
function sortSchedulesSmart(schedules) {
    ...
    return schedules.sort((a, b) => {
        ...
        // ĐẢO NGƯỢC LOGIC: Nếu một trong hai đã hết hạn, đẩy thằng hết hạn lên TRÊN (-1)
        if (isPastA && !isPastB) return -1;
        if (!isPastA && isPastB) return 1;

        // Nếu cả hai cùng trạng thái (cùng chưa hết hạn, hoặc cùng đã hết hạn)
        const startA = new Date(`${a.date}T${a.time || "00:00"}`);
        const startB = new Date(`${b.date}T${b.time || "00:00"}`);
        
        const effectiveA = (now >= startA && now <= finalTimeA) ? now : startA;
        const effectiveB = (now >= startB && now <= finalTimeB) ? now : startB;
